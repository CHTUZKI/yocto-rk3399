...
static void rkclk_init(struct rockchip_cru *cru)
{
...
#if DEBUG_SERIAL
	debug_serial_puts("\r\n[CLK] ===== rkclk_init() START =====\r\n");
	debug_serial_puts("[CLK] CRITICAL: This function will reconfigure ALL clocks!\r\n");
	debug_serial_puts("[CLK] UART clock (24MHz OSC) may be affected if PERILP clocks change!\r\n");
	debug_serial_puts("[CLK] Current OSC_HZ: ");
	debug_serial_put_ulong(OSC_HZ);
	debug_serial_puts(" Hz (24MHz)\r\n");
	debug_serial_puts("[CLK] UART2 should use OSC_HZ (24MHz) as clock source\r\n");
	debug_serial_puts("[CLK] If PERILP PCLK changes, UART baudrate calculation may be affected!\r\n");
#endif

	rk3399_configure_cpu_l(cru, APLL_L_600_MHZ);
#if DEBUG_SERIAL
	debug_serial_puts("[CLK] CPU L cluster configured (APLL_L_600_MHZ)\r\n");
#endif

	rk3399_configure_cpu_b(cru, APLL_B_600_MHZ);
#if DEBUG_SERIAL
	debug_serial_puts("[CLK] CPU B cluster configured (APLL_B_600_MHZ)\r\n");
#endif
	 ...
	 */
#if DEBUG_SERIAL
	debug_serial_puts("[CLK] Resetting bootrom-modified CRU registers...\r\n");
	debug_serial_puts("[CLK] clksel_con[12] = 0x4101\r\n");
	debug_serial_puts("[CLK] clksel_con[19] = 0x033f\r\n");
	debug_serial_puts("[CLK] clksel_con[56] = 0x0003\r\n");
#endif
	...
	/* configure gpll cpll */
#if DEBUG_SERIAL
	debug_serial_puts("[CLK] Configuring GPLL...\r\n");
	debug_serial_puts("[CLK] GPLL target: ");
	debug_serial_put_ulong(GPLL_HZ);
	debug_serial_puts(" Hz\r\n");
#endif
	rkclk_set_pll(&cru->gpll_con[0], &gpll_init_cfg);
#if DEBUG_SERIAL
	debug_serial_puts("[CLK] GPLL configured\r\n");
#endif

#if DEBUG_SERIAL
	debug_serial_puts("[CLK] Configuring CPLL...\r\n");
	debug_serial_puts("[CLK] CPLL target: ");
	debug_serial_put_ulong(CPLL_HZ);
	debug_serial_puts(" Hz\r\n");
#endif
	rkclk_set_pll(&cru->cpll_con[0], &cpll_init_cfg);
#if DEBUG_SERIAL
	debug_serial_puts("[CLK] CPLL configured\r\n");
#endif

	/* configure perihp aclk, hclk, pclk */
#if DEBUG_SERIAL
	debug_serial_puts("[CLK] Configuring PERIHP clocks (ACLK, HCLK, PCLK)...\r\n");
#endif
	aclk_div = GPLL_HZ / PERIHP_ACLK_HZ - 1;
	assert((aclk_div + 1) * PERIHP_ACLK_HZ == GPLL_HZ && aclk_div < 0x1f);
#if DEBUG_SERIAL
	debug_serial_puts("[CLK] PERIHP ACLK: ");
	debug_serial_put_ulong(PERIHP_ACLK_HZ);
	debug_serial_puts(" Hz, div=");
	debug_serial_put_ulong(aclk_div);
	debug_serial_puts("\r\n");
#endif
...
#if DEBUG_SERIAL
	debug_serial_puts("[CLK] PERIHP HCLK: ");
	debug_serial_put_ulong(PERIHP_HCLK_HZ);
	debug_serial_puts(" Hz, div=");
	debug_serial_put_ulong(hclk_div);
	debug_serial_puts("\r\n");
#endif
...
#if DEBUG_SERIAL
	debug_serial_puts("[CLK] PERIHP PCLK: ");
	debug_serial_put_ulong(PERIHP_PCLK_HZ);
	debug_serial_puts(" Hz, div=");
	debug_serial_put_ulong(pclk_div);
	debug_serial_puts("\r\n");
	debug_serial_puts("[CLK] Writing PERIHP clocks to clksel_con[14]...\r\n");
#endif
...
#if DEBUG_SERIAL
	debug_serial_puts("[CLK] PERIHP clocks configured\r\n");
#endif

	/* configure perilp0 aclk, hclk, pclk */
#if DEBUG_SERIAL
	debug_serial_puts("[CLK] ===== CRITICAL: Configuring PERILP0 clocks =====\r\n");
	debug_serial_puts("[CLK] WARNING: PERILP0 PCLK affects UART and other peripherals!\r\n");
	debug_serial_puts("[CLK] UART uses OSC_HZ (24MHz) directly, but PERILP0 PCLK may affect UART registers!\r\n");
#endif
	aclk_div = GPLL_HZ / PERILP0_ACLK_HZ - 1;
	assert((aclk_div + 1) * PERILP0_ACLK_HZ == GPLL_HZ && aclk_div < 0x1f);
#if DEBUG_SERIAL
	debug_serial_puts("[CLK] PERILP0 ACLK: ");
	debug_serial_put_ulong(PERILP0_ACLK_HZ);
	debug_serial_puts(" Hz, div=");
	debug_serial_put_ulong(aclk_div);
	debug_serial_puts("\r\n");
#endif
...
#if DEBUG_SERIAL
	debug_serial_puts("[CLK] PERILP0 HCLK: ");
	debug_serial_put_ulong(PERILP0_HCLK_HZ);
	debug_serial_puts(" Hz, div=");
	debug_serial_put_ulong(hclk_div);
	debug_serial_puts("\r\n");
#endif
...
#if DEBUG_SERIAL
	debug_serial_puts("[CLK] PERILP0 PCLK: ");
	debug_serial_put_ulong(PERILP0_PCLK_HZ);
	debug_serial_puts(" Hz, div=");
	debug_serial_put_ulong(pclk_div);
	debug_serial_puts("\r\n");
	debug_serial_puts("[CLK] CRITICAL: About to write PERILP0 clocks to clksel_con[23]!\r\n");
	debug_serial_puts("[CLK] This may affect UART register access timing!\r\n");
#endif
...
#if DEBUG_SERIAL
	debug_serial_puts("[CLK] PERILP0 clocks configured\r\n");
	debug_serial_puts("[CLK] ===== END PERILP0 configuration =====\r\n");
#endif

	/* perilp1 hclk select gpll as source */
#if DEBUG_SERIAL
	debug_serial_puts("[CLK] Configuring PERILP1 clocks...\r\n");
#endif
	hclk_div = GPLL_HZ / PERILP1_HCLK_HZ - 1;
	assert((hclk_div + 1) * PERILP1_HCLK_HZ ==
	       GPLL_HZ && (hclk_div < 0x1f));
#if DEBUG_SERIAL
	debug_serial_puts("[CLK] PERILP1 HCLK: ");
	debug_serial_put_ulong(PERILP1_HCLK_HZ);
	debug_serial_puts(" Hz, div=");
	debug_serial_put_ulong(hclk_div);
	debug_serial_puts("\r\n");
#endif
...
#if DEBUG_SERIAL
	debug_serial_puts("[CLK] PERILP1 PCLK: ");
	debug_serial_put_ulong(PERILP1_HCLK_HZ);
	debug_serial_puts(" Hz, div=");
	debug_serial_put_ulong(pclk_div);
	debug_serial_puts("\r\n");
	debug_serial_puts("[CLK] Writing PERILP1 clocks to clksel_con[25]...\r\n");
#endif
...
#if DEBUG_SERIAL
	debug_serial_puts("[CLK] PERILP1 clocks configured\r\n");
	debug_serial_puts("[CLK] ===== rkclk_init() COMPLETED =====\r\n");
	debug_serial_puts("[CLK] CRITICAL: All clocks have been reconfigured!\r\n");
	debug_serial_puts("[CLK] UART clock source (OSC_HZ=24MHz) should remain unchanged\r\n");
	debug_serial_puts("[CLK] However, if UART baudrate was calculated based on wrong clock,\r\n");
	debug_serial_puts("[CLK] serial output may become garbled after this point!\r\n");
	debug_serial_puts("[CLK] ===== END rkclk_init() =====\r\n\r\n");
#endif
}
...
static int rk3399_clk_probe(struct udevice *dev)
{
...
#if DEBUG_SERIAL
	debug_serial_puts("\r\n[CLK] ===== rk3399_clk_probe() START =====\r\n");
	debug_serial_puts("[CLK] CRITICAL: Clock controller is being probed!\r\n");
	debug_serial_puts("[CLK] This happens AFTER PMIC initialization!\r\n");
	debug_serial_puts("[CLK] Current baudrate: ");
	if (gd) {
	...
	debug_serial_puts("\r\n");
	debug_serial_puts("[CLK] Serial device state: ");
	...
	debug_serial_puts("\r\n");
	debug_serial_puts("[CLK] init_clocks: ");
	debug_serial_put_ulong(init_clocks ? 1 : 0);
	debug_serial_puts("\r\n");
	debug_serial_puts("[CLK] CRITICAL: Even if init_clocks=0, clock controller probe may affect UART!\r\n");
	debug_serial_puts("[CLK] Checking if priv->cru is valid...\r\n");
	if (priv && priv->cru) {
		debug_serial_puts("[CLK] priv->cru is valid, address: ");
		debug_serial_put_ulong((unsigned long)priv->cru);
		debug_serial_puts("\r\n");
		debug_serial_puts("[CLK] WARNING: Clock controller registers are accessible!\r\n");
		debug_serial_puts("[CLK] Any register read/write might affect clock configuration!\r\n");
	} else {
		debug_serial_puts("[CLK] priv->cru is NULL or invalid\r\n");
	}
	debug_serial_puts("[CLK] ===== BEFORE any clock operations =====\r\n");
#endif

	if (init_clocks)
#if DEBUG_SERIAL
	{
		debug_serial_puts("\r\n[CLK] ===== CRITICAL POINT: About to call rkclk_init() =====\r\n");
		debug_serial_puts("[CLK] WARNING: This will reconfigure ALL clocks!\r\n");
		debug_serial_puts("[CLK] UART clock source (OSC_HZ=24MHz) should NOT change\r\n");
		debug_serial_puts("[CLK] But PERILP clocks will change, which may affect UART register access!\r\n");
		debug_serial_puts("[CLK] Current gd->baudrate BEFORE rkclk_init: ");
		if (gd) {
			debug_serial_put_ulong(gd->baudrate);
		} else {
			debug_serial_puts("unknown (gd is NULL)");
		...
		debug_serial_puts("\r\n");
		debug_serial_puts("[CLK] Expected UART clock: 24MHz (OSC_HZ)\r\n");
		debug_serial_puts("[CLK] Expected baudrate: 1500000\r\n");
		debug_serial_puts("[CLK] If baudrate != 1500000, serial output will be garbled!\r\n");
		debug_serial_puts("[CLK] ===== CALLING rkclk_init() NOW =====\r\n");
#endif
		rkclk_init(priv->cru);
#if DEBUG_SERIAL
		debug_serial_puts("\r\n[CLK] ===== RETURNED FROM rkclk_init() =====\r\n");
		debug_serial_puts("[CLK] rkclk_init() completed\r\n");
		debug_serial_puts("[CLK] Current gd->baudrate AFTER rkclk_init: ");
		if (gd) {
			debug_serial_put_ulong(gd->baudrate);
		} else {
			debug_serial_puts("unknown (gd is NULL)");
		...
		debug_serial_puts("\r\n");
		debug_serial_puts("[CLK] CRITICAL CHECK: If baudrate != 1500000, serial is broken!\r\n");
		if (gd && gd->baudrate != 1500000) {
			debug_serial_puts("[CLK] ERROR: Baudrate mismatch! Expected 1500000, got ");
			debug_serial_put_ulong(gd->baudrate);
			debug_serial_puts("\r\n");
			debug_serial_puts("[CLK] This will cause garbled serial output!\r\n");
		} else if (gd) {
			debug_serial_puts("[CLK] OK: Baudrate is correct (1500000)\r\n");
		}
		debug_serial_puts("[CLK] UART clock source should still be OSC_HZ (24MHz)\r\n");
		debug_serial_puts("[CLK] ===== END CRITICAL POINT =====\r\n\r\n");
	}
#endif

#if DEBUG_SERIAL
	debug_serial_puts("\r\n[CLK] ===== BEFORE RETURN FROM rk3399_clk_probe() =====\r\n");
	debug_serial_puts("[CLK] CRITICAL: About to return from clock controller probe\r\n");
	debug_serial_puts("[CLK] Current baudrate: ");
	if (gd) {
		debug_serial_put_ulong(gd->baudrate);
		if (gd->baudrate != 1500000) {
			debug_serial_puts("\r\n[CLK] ERROR: Baudrate changed during clock probe!\r\n");
			debug_serial_puts("[CLK] Expected 1500000, got ");
			debug_serial_put_ulong(gd->baudrate);
			debug_serial_puts("\r\n");
		} else {
	...
	debug_serial_puts("\r\n");
	debug_serial_puts("[CLK] ===== rk3399_clk_probe() END =====\r\n");
	debug_serial_puts("[CLK] ===== ACTUALLY RETURNING FROM rk3399_clk_probe() =====\r\n\r\n");
#endif
