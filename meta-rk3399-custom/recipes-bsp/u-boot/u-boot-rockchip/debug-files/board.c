...
{
#if DEBUG_SERIAL
	debug_serial_puts("\r\n[BOARD_LATE_INIT] board_late_init() START\r\n");
	debug_serial_puts("[BOARD_LATE_INIT] Current baudrate: ");
	if (gd) {
	...
	debug_serial_puts("\r\n");
	debug_serial_puts("[BOARD_LATE_INIT] About to call setup_boot_mode()\r\n");
#endif

	setup_boot_mode();

#if DEBUG_SERIAL
	debug_serial_puts("[BOARD_LATE_INIT] After setup_boot_mode()\r\n");
	debug_serial_puts("[BOARD_LATE_INIT] Current baudrate: ");
	if (gd) {
		debug_serial_put_ulong(gd->baudrate);
	} else {
...
#if IS_ENABLED(CONFIG_EFI_HAVE_CAPSULE_SUPPORT) && IS_ENABLED(CONFIG_EFI_PARTITION)
#if DEBUG_SERIAL
	debug_serial_puts("[BOARD_LATE_INIT] About to call gpt_capsule_update_setup()\r\n");
#endif
	gpt_capsule_update_setup();
#if DEBUG_SERIAL
	debug_serial_puts("[BOARD_LATE_INIT] After gpt_capsule_update_setup()\r\n");
	debug_serial_puts("[BOARD_LATE_INIT] Current baudrate: ");
	if (gd) {
		debug_serial_put_ulong(gd->baudrate);
	} else {
...
#if DEBUG_SERIAL
	debug_serial_puts("[BOARD_LATE_INIT] About to call rk_board_late_init()\r\n");
#endif
	int ret = rk_board_late_init();

#if DEBUG_SERIAL
	debug_serial_puts("[BOARD_LATE_INIT] After rk_board_late_init(), return value: ");
	...
	debug_serial_puts("\r\n");
	debug_serial_puts("[BOARD_LATE_INIT] board_late_init() END\r\n");
#endif
