...
static int rk8xx_probe(struct udevice *dev)
{
	...
	if (!IS_ENABLED(CONFIG_SPL_BUILD)) {
#if DEBUG_SERIAL
		debug_serial_puts("\r\n[PMIC] About to print PMIC info\r\n");
		debug_serial_puts("[PMIC] Current baudrate: ");
		if (gd) {
		...
		printf("\n");
#if DEBUG_SERIAL
		debug_serial_puts("[PMIC] PMIC info printed\r\n");
		debug_serial_puts("[PMIC] Current baudrate after printf: ");
		if (gd) {
			debug_serial_put_ulong(gd->baudrate);
		} else {
			debug_serial_puts("unknown");
		...
		/* CRITICAL DEBUG: After PMIC print - check serial device state */
		debug_serial_puts("[PMIC] ===== CRITICAL DEBUG POINT =====\r\n");
		debug_serial_puts("[PMIC] Checking serial device state...\r\n");
		if (gd && gd->cur_serial_dev) {
			debug_serial_puts("[PMIC] Serial device exists: ");
			...
			debug_serial_puts("[PMIC] WARNING: Serial device is NULL!\r\n");
		}
		debug_serial_puts("[PMIC] Serial flags: ");
		if (gd) {
			debug_serial_put_ulong(gd->flags);
		} else {
			debug_serial_puts("unknown");
		}
		debug_serial_puts("\r\n");
		debug_serial_puts("[PMIC] About to return from rk8xx_probe()\r\n");
		debug_serial_puts("[PMIC] ===== END CRITICAL DEBUG =====\r\n");
#endif
	}
...
#if DEBUG_SERIAL
	debug_serial_puts("[PMIC] rk8xx_probe() returning 0\r\n");
	debug_serial_puts("[PMIC] Final baudrate check: ");
	if (gd) {
		debug_serial_put_ulong(gd->baudrate);
	} else {
		debug_serial_puts("unknown");
	...
	debug_serial_puts("\r\n");
	debug_serial_puts("[PMIC] ===== CRITICAL: About to return from rk8xx_probe() =====\r\n");
	debug_serial_puts("[PMIC] WARNING: After this return, dm_init_and_scan() will continue!\r\n");
	debug_serial_puts("[PMIC] Other devices may be initialized after PMIC!\r\n");
	debug_serial_puts("[PMIC] CRITICAL: Clock controller will be probed next!\r\n");
	debug_serial_puts("[PMIC] Clock controller probe may affect UART configuration!\r\n");
	debug_serial_puts("[PMIC] Current baudrate before return: ");
	if (gd) {
		debug_serial_put_ulong(gd->baudrate);
	} else {
		debug_serial_puts("unknown");
	...
	debug_serial_puts("\r\n");
	debug_serial_puts("[PMIC] Serial device state: ");
	if (gd && gd->cur_serial_dev) {
		if (gd->cur_serial_dev->name) {
			debug_serial_puts(gd->cur_serial_dev->name);
		} else {
	...
	debug_serial_puts("\r\n");
	debug_serial_puts("[PMIC] ===== END CRITICAL WARNING =====\r\n");
	debug_serial_puts("[PMIC] ===== ACTUALLY RETURNING FROM rk8xx_probe() NOW =====\r\n");
	/* Force flush - send multiple newlines to ensure output is flushed */
	debug_serial_puts("\r\n\r\n\r\n");
#endif
