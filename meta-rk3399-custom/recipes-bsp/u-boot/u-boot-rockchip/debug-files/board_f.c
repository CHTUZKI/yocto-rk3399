...
{
#if DEBUG_SERIAL
	debug_serial_puts("\r\n[DRAM] announce_dram_init() called\r\n");
	debug_serial_puts("[DRAM] Current baudrate: ");
	if (gd) {
	...
	debug_serial_puts("\r\n");
	debug_serial_puts("[DRAM] About to print DRAM: \r\n");
#endif
	puts("DRAM:  ");
#if DEBUG_SERIAL
	debug_serial_puts("[DRAM] DRAM: printed\r\n");
#endif
	return 0;
}
...
static int show_dram_config(void)
{
...
#if DEBUG_SERIAL
	debug_serial_puts("[DRAM] show_dram_config() - About to print DRAM size\r\n");
	debug_serial_puts("[DRAM] Current baudrate: ");
	if (gd) {
		debug_serial_put_ulong(gd->baudrate);
	} else {
	...
	debug_serial_puts("\r\n");
	debug_serial_puts("[DRAM] ram_size: ");
	if (gd) {
		debug_serial_put_ulong(gd->ram_size);
	} else {
		debug_serial_puts("unknown");
	}
	debug_serial_puts("\r\n");
...
#if DEBUG_SERIAL
	debug_serial_puts("[DRAM] DRAM size printed\r\n");
	debug_serial_puts("[DRAM] Current baudrate after print_size: ");
	if (gd) {
		debug_serial_put_ulong(gd->baudrate);
	} else {
		debug_serial_puts("unknown");
