...
static inline void debug_serial_putc_internal(char c)
{
...
{
	debug_serial_putc_internal(c);
}

static void debug_serial_puts(const char *s)
{
	while (*s)
		debug_serial_putc_internal(*s++);
}
...
static void debug_serial_put_ulong(unsigned long val)
{
	...
	while (i > 0)
		debug_serial_putc_internal(buf[--i]);
}
#else
static inline void debug_serial_putc(char c) {}
static inline void debug_serial_putc_internal(char c) {}
...
const char *bootdelay_process(void)
{
...
#if DEBUG_SERIAL
	debug_serial_puts("\r\n[MAIN_LOOP] ===== main_loop() START =====\r\n");
	debug_serial_puts("[MAIN_LOOP] Current baudrate: ");
	if (gd) {
	...
	if (gd && gd->cur_serial_dev) {
		debug_serial_puts("[MAIN_LOOP] Serial device: ");
		...
		debug_serial_puts("[MAIN_LOOP] WARNING: Serial device is NULL!\r\n");
	}
	
	debug_serial_puts("[MAIN_LOOP] bootdelay: ");
	debug_serial_put_ulong(bootdelay);
	debug_serial_puts("\r\n");
#endif
...
#if DEBUG_SERIAL
		debug_serial_puts("[MAIN_LOOP] Using failbootcmd\r\n");
#endif
	} else
#endif /* CONFIG_POST */
	if (bootcount_error()) {
...
#if DEBUG_SERIAL
		debug_serial_puts("[MAIN_LOOP] Using altbootcmd (bootcount error)\r\n");
#endif
	} else {
		s = env_get("bootcmd");
#if DEBUG_SERIAL
		debug_serial_puts("[MAIN_LOOP] Using bootcmd\r\n");
#endif
	}

#if DEBUG_SERIAL
	if (s) {
		debug_serial_puts("[MAIN_LOOP] bootcmd value: ");
		debug_serial_puts(s);
		debug_serial_puts("\r\n");
	} else {
		debug_serial_puts("[MAIN_LOOP] WARNING: bootcmd is NULL!\r\n");
	}
#endif
...
#if DEBUG_SERIAL
	debug_serial_puts("[MAIN_LOOP] About to return from main_loop()\r\n");
	debug_serial_puts("[MAIN_LOOP] Final baudrate: ");
	if (gd) {
		debug_serial_put_ulong(gd->baudrate);
	} else {
		debug_serial_puts("unknown");
	...
	debug_serial_puts("\r\n");
	debug_serial_puts("[MAIN_LOOP] ===== main_loop() END =====\r\n");
#endif
...
void autoboot_command(const char *s)
{
...
#if DEBUG_SERIAL
	debug_serial_puts("\r\n[AUTOBOOT] ===== autoboot_command() START =====\r\n");
	debug_serial_puts("[AUTOBOOT] Current baudrate: ");
	if (gd) {
		debug_serial_put_ulong(gd->baudrate);
	} else {
		debug_serial_puts("unknown");
	...
	/* Check serial device state */
	if (gd && gd->cur_serial_dev) {
		debug_serial_puts("[AUTOBOOT] Serial device: ");
		if (gd->cur_serial_dev->name) {
			debug_serial_puts(gd->cur_serial_dev->name);
		} else {
			debug_serial_puts("NULL name");
		...
		debug_serial_puts("[AUTOBOOT] WARNING: Serial device is NULL!\r\n");
	}
	
	if (s) {
		debug_serial_puts("[AUTOBOOT] bootcmd: ");
		debug_serial_puts(s);
		debug_serial_puts("\r\n");
	} else {
		debug_serial_puts("[AUTOBOOT] bootcmd: <UNDEFINED>\r\n");
	}
	
	debug_serial_puts("[AUTOBOOT] stored_bootdelay: ");
	debug_serial_put_ulong(stored_bootdelay);
	debug_serial_puts("\r\n");
#endif
	...
	if (s && (stored_bootdelay == -2 ||
...
#if DEBUG_SERIAL
		debug_serial_puts("[AUTOBOOT] Condition met: will execute bootcmd\r\n");
		debug_serial_puts("[AUTOBOOT] About to call abortboot()\r\n");
#endif
...
#if DEBUG_SERIAL
		debug_serial_puts("[AUTOBOOT] About to call run_command_list()\r\n");
		debug_serial_puts("[AUTOBOOT] Current baudrate before run_command_list: ");
		if (gd) {
			debug_serial_put_ulong(gd->baudrate);
		} else {
			debug_serial_puts("unknown");
		...
		debug_serial_puts("\r\n");
		debug_serial_puts("[AUTOBOOT] ===== EXECUTING bootcmd =====\r\n");
#endif

		run_command_list(s, -1, 0);

#if DEBUG_SERIAL
		debug_serial_puts("[AUTOBOOT] ===== bootcmd EXECUTION COMPLETED =====\r\n");
		debug_serial_puts("[AUTOBOOT] Current baudrate after run_command_list: ");
		if (gd) {
			debug_serial_put_ulong(gd->baudrate);
		} else {
			debug_serial_puts("unknown");
		...
		/* Check serial device state after command execution */
		if (gd && gd->cur_serial_dev) {
			debug_serial_puts("[AUTOBOOT] Serial device after command: ");
			if (gd->cur_serial_dev->name) {
				debug_serial_puts(gd->cur_serial_dev->name);
			} else {
				debug_serial_puts("NULL name");
			...
			debug_serial_puts("[AUTOBOOT] WARNING: Serial device is NULL after command!\r\n");
		}
#endif
...
#if DEBUG_SERIAL
		debug_serial_puts("[AUTOBOOT] Condition NOT met: will NOT execute bootcmd\r\n");
		if (!s) {
			debug_serial_puts("[AUTOBOOT] Reason: bootcmd is NULL\r\n");
		} else if (stored_bootdelay == -1) {
			debug_serial_puts("[AUTOBOOT] Reason: bootdelay is -1 (disabled)\r\n");
		} else {
			debug_serial_puts("[AUTOBOOT] Reason: abortboot() returned true\r\n");
		}
#endif
	}
...
#if DEBUG_SERIAL
	debug_serial_puts("[AUTOBOOT] ===== autoboot_command() END =====\r\n");
	debug_serial_puts("[AUTOBOOT] Final baudrate: ");
	if (gd) {
		debug_serial_put_ulong(gd->baudrate);
	} else {
		debug_serial_puts("unknown");
