...
static int rockchip_emmc_phy_power(struct phy *phy, bool on_off)
{
	...
	pr_info("[DEBUG] rk3399: rockchip_emmc_phy_power: Starting, on_off=%d\n", on_off);
	/*
	 * Keep phyctrl_pdb and phyctrl_endll low to allow
	 * initialization of CALIO state M/C DFFs
	 */
	pr_info("[DEBUG] rk3399: rockchip_emmc_phy_power: Setting PHYCTRL_PDB_PWR_OFF\n");
	regmap_write(rk_phy->reg_base,
		     rk_phy->reg_offset + GRF_EMMCPHY_CON6,
		     HIWORD_UPDATE(PHYCTRL_PDB_PWR_OFF,
				   PHYCTRL_PDB_MASK,
				   PHYCTRL_PDB_SHIFT));
	pr_info("[DEBUG] rk3399: rockchip_emmc_phy_power: Setting PHYCTRL_ENDLL_DISABLE\n");
	regmap_write(rk_phy->reg_base,
		     rk_phy->reg_offset + GRF_EMMCPHY_CON6,
	...
	if (on_off == PHYCTRL_PDB_PWR_OFF) {
		pr_info("[DEBUG] rk3399: rockchip_emmc_phy_power: Power off completed\n");
		return 0;
	}

	pr_info("[DEBUG] rk3399: rockchip_emmc_phy_power: Getting emmcclk rate\n");
	rate = clk_get_rate(rk_phy->emmcclk);
	pr_info("[DEBUG] rk3399: rockchip_emmc_phy_power: emmcclk rate=%lu\n", rate);
	 ...
	 */
	pr_info("[DEBUG] rk3399: rockchip_emmc_phy_power: Delaying 3us before power on\n");
	udelay(3);
	pr_info("[DEBUG] rk3399: rockchip_emmc_phy_power: Setting PHYCTRL_PDB_PWR_ON\n");
	regmap_write(rk_phy->reg_base,
		     rk_phy->reg_offset + GRF_EMMCPHY_CON6,
	 ...
	 */
	pr_info("[DEBUG] rk3399: rockchip_emmc_phy_power: Polling for CALDONE\n");
	...
	if (ret) {
		...
		pr_info("[DEBUG] rk3399: rockchip_emmc_phy_power: CALDONE polling failed, ret=%d\n", ret);
		return ret;
	}

	/* Set the frequency of the DLL operation */
	pr_info("[DEBUG] rk3399: rockchip_emmc_phy_power: Setting DLL frequency, freqsel=%u\n", freqsel);
	regmap_write(rk_phy->reg_base,
	...
	/* Turn on the DLL */
	pr_info("[DEBUG] rk3399: rockchip_emmc_phy_power: Enabling DLL\n");
	regmap_write(rk_phy->reg_base,
		     rk_phy->reg_offset + GRF_EMMCPHY_CON6,
	 ...
	 */
	if (rate == 0) {
		pr_info("[DEBUG] rk3399: rockchip_emmc_phy_power: Rate is 0, returning\n");
		return 0;
	}
	 ...
	 */
	pr_info("[DEBUG] rk3399: rockchip_emmc_phy_power: Polling for DLLRDY\n");
	ret = regmap_read_poll_timeout(rk_phy->reg_base,
				       rk_phy->reg_offset + GRF_EMMCPHY_STATUS,
	...
	if (ret) {
		...
		pr_info("[DEBUG] rk3399: rockchip_emmc_phy_power: DLLRDY polling failed, ret=%d\n", ret);
		return ret;
	}
...
static int rockchip_emmc_phy_init(struct phy *phy)
{
	...
	pr_info("[DEBUG] rk3399: rockchip_emmc_phy_init: Starting\n");
	/*
	 ...
	 */
	pr_info("[DEBUG] rk3399: rockchip_emmc_phy_init: Getting emmcclk\n");
	rk_phy->emmcclk = clk_get_optional(&phy->dev, "emmcclk");
	if (IS_ERR(rk_phy->emmcclk)) {
		ret = PTR_ERR(rk_phy->emmcclk);
		dev_err(&phy->dev, "Error getting emmcclk: %d\n", ret);
		pr_info("[DEBUG] rk3399: rockchip_emmc_phy_init: Error getting emmcclk: %d\n", ret);
		rk_phy->emmcclk = NULL;
	} else {
		pr_info("[DEBUG] rk3399: rockchip_emmc_phy_init: Successfully got emmcclk\n");
	}

	pr_info("[DEBUG] rk3399: rockchip_emmc_phy_init: Completed, ret=%d\n", ret);
	return ret;
}
...
{
	int ret;
	pr_info("[DEBUG] rk3399: rockchip_emmc_phy_power_off: Starting\n");
	/* Power down emmc phy analog blocks */
	ret = rockchip_emmc_phy_power(phy, PHYCTRL_PDB_PWR_OFF);
	pr_info("[DEBUG] rk3399: rockchip_emmc_phy_power_off: Completed, ret=%d\n", ret);
	return ret;
}
...
static int rockchip_emmc_phy_power_on(struct phy *phy)
{
	...
	pr_info("[DEBUG] rk3399: rockchip_emmc_phy_power_on: Starting\n");

	/* Drive impedance: from DTS */
	pr_info("[DEBUG] rk3399: rockchip_emmc_phy_power_on: Setting drive impedance\n");
	regmap_write(rk_phy->reg_base,
		     rk_phy->reg_offset + GRF_EMMCPHY_CON6,
	...
	pr_info("[DEBUG] rk3399: rockchip_emmc_phy_power_on: Enabling output tap delay\n");
	regmap_write(rk_phy->reg_base,
		     rk_phy->reg_offset + GRF_EMMCPHY_CON0,
	...
	/* Output tap delay */
	pr_info("[DEBUG] rk3399: rockchip_emmc_phy_power_on: Setting output tap delay\n");
	regmap_write(rk_phy->reg_base,
		     rk_phy->reg_offset + GRF_EMMCPHY_CON0,
	...
	pr_info("[DEBUG] rk3399: rockchip_emmc_phy_power_on: Setting strobe pull-down\n");
	regmap_write(rk_phy->reg_base,
	...
	pr_info("[DEBUG] rk3399: rockchip_emmc_phy_power_on: Calling rockchip_emmc_phy_power\n");
	ret = rockchip_emmc_phy_power(phy, PHYCTRL_PDB_PWR_ON);
	pr_info("[DEBUG] rk3399: rockchip_emmc_phy_power_on: Completed, ret=%d\n", ret);
	return ret;
}
...
static int rockchip_emmc_phy_probe(struct platform_device *pdev)
{
	...
	pr_info("[DEBUG] rk3399: rockchip_emmc_phy_probe: Starting\n");
	if (!dev->parent || !dev->parent->of_node) {
		pr_info("[DEBUG] rk3399: rockchip_emmc_phy_probe: No parent or of_node\n");
		return -ENODEV;
	}

	pr_info("[DEBUG] rk3399: rockchip_emmc_phy_probe: Getting GRF regmap\n");
	grf = syscon_node_to_regmap(dev->parent->of_node);
	if (IS_ERR(grf)) {
		dev_err(dev, "Missing rockchip,grf property\n");
		pr_info("[DEBUG] rk3399: rockchip_emmc_phy_probe: Failed to get GRF, ret=%ld\n", PTR_ERR(grf));
		return PTR_ERR(grf);
	}
	pr_info("[DEBUG] rk3399: rockchip_emmc_phy_probe: Successfully got GRF regmap\n");
	...
	pr_info("[DEBUG] rk3399: rockchip_emmc_phy_probe: Creating PHY\n");
	generic_phy = devm_phy_create(dev, dev->of_node, &ops);
	if (IS_ERR(generic_phy)) {
		dev_err(dev, "failed to create PHY\n");
		pr_info("[DEBUG] rk3399: rockchip_emmc_phy_probe: Failed to create PHY, ret=%ld\n", PTR_ERR(generic_phy));
		return PTR_ERR(generic_phy);
	}

	phy_set_drvdata(generic_phy, rk_phy);
	pr_info("[DEBUG] rk3399: rockchip_emmc_phy_probe: Registering PHY provider\n");
	phy_provider = devm_of_phy_provider_register(dev, of_phy_simple_xlate);

	pr_info("[DEBUG] rk3399: rockchip_emmc_phy_probe: Completed\n");
