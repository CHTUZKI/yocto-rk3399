...
static irqreturn_t rk_timer_interrupt(int irq, void *dev_id)
{
	...
	pr_info("[DEBUG] rk3399: rk_timer_interrupt: Entry, irq=%d\n", irq);
	rk_timer_interrupt_clear(timer);
	pr_info("[DEBUG] rk3399: rk_timer_interrupt: Interrupt cleared\n");

	if (clockevent_state_oneshot(ce)) {
		pr_info("[DEBUG] rk3399: rk_timer_interrupt: Oneshot mode, disabling timer\n");
		rk_timer_disable(timer);
	}

	pr_info("[DEBUG] rk3399: rk_timer_interrupt: Calling event_handler=%p\n", ce->event_handler);
	ce->event_handler(ce);
	pr_info("[DEBUG] rk3399: rk_timer_interrupt: event_handler returned\n");

	pr_info("[DEBUG] rk3399: rk_timer_interrupt: Returning IRQ_HANDLED\n");
	return IRQ_HANDLED;
}
...
rk_timer_probe(struct rk_timer *timer, struct device_node *np)
{
	...
	pr_info("[DEBUG] rk3399: rk_timer_probe: Starting for %s\n", np->full_name);
	...
	rk_timer_interrupt_clear(timer);
	rk_timer_disable(timer);
	pr_info("[DEBUG] rk3399: rk_timer_probe: Completed successfully, freq=%u Hz, irq=%d\n", timer->freq, timer->irq);
	return 0;
...
static int __init rk_clkevt_init(struct device_node *np)
{
	...
	pr_info("[DEBUG] rk3399: rk_clkevt_init: Requesting IRQ %d\n", rk_clkevt->timer.irq);
	ret = request_irq(rk_clkevt->timer.irq, rk_timer_interrupt, IRQF_TIMER,
			  TIMER_NAME, ce);
	if (ret) {
		pr_err("[DEBUG] rk3399: rk_clkevt_init: Failed to initialize '%s': %d\n",
			TIMER_NAME, ret);
		goto out_irq;
	}
	pr_info("[DEBUG] rk3399: rk_clkevt_init: IRQ %d registered successfully\n", rk_clkevt->timer.irq);

	pr_info("[DEBUG] rk3399: rk_clkevt_init: Calling clockevents_config_and_register, freq=%u\n", rk_clkevt->timer.freq);
	clockevents_config_and_register(&rk_clkevt->ce,
					rk_clkevt->timer.freq, 1, UINT_MAX);
	pr_info("[DEBUG] rk3399: rk_clkevt_init: clockevents_config_and_register returned\n");
	return 0;
