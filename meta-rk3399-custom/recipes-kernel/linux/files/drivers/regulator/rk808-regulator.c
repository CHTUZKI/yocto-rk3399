...
static int rk808_regulator_probe(struct platform_device *pdev)
{
	...
	pr_info("[DEBUG] rk3399: rk808_regulator_probe: Starting\n");
	pdata = devm_kzalloc(&pdev->dev, sizeof(*pdata), GFP_KERNEL);
	if (!pdata) {
		pr_info("[DEBUG] rk3399: rk808_regulator_probe: Failed to allocate memory\n");
		return -ENOMEM;
	}

	pr_info("[DEBUG] rk3399: rk808_regulator_probe: Parsing device tree data\n");
	ret = rk808_regulator_dt_parse_pdata(&pdev->dev, &client->dev,
					     rk808->regmap, pdata);
	if (ret < 0) {
		pr_info("[DEBUG] rk3399: rk808_regulator_probe: Failed to parse DT data, ret=%d\n", ret);
		return ret;
	}

	platform_set_drvdata(pdev, pdata);

	pr_info("[DEBUG] rk3399: rk808_regulator_probe: Variant=%lu\n", rk808->variant);
	switch (rk808->variant) {
	case RK805_ID:
		regulators = rk805_reg;
		nregulators = RK805_NUM_REGULATORS;
		pr_info("[DEBUG] rk3399: rk808_regulator_probe: Using RK805 regulators\n");
		break;
	case RK808_ID:
		regulators = rk808_reg;
		nregulators = RK808_NUM_REGULATORS;
		pr_info("[DEBUG] rk3399: rk808_regulator_probe: Using RK808 regulators\n");
		break;
	case RK809_ID:
		regulators = rk809_reg;
		nregulators = RK809_NUM_REGULATORS;
		pr_info("[DEBUG] rk3399: rk808_regulator_probe: Using RK809 regulators\n");
		break;
	case RK816_ID:
		regulators = rk816_reg;
		nregulators = RK816_NUM_REGULATORS;
		pr_info("[DEBUG] rk3399: rk808_regulator_probe: Using RK816 regulators\n");
		break;
	case RK817_ID:
		regulators = rk817_reg;
		nregulators = RK817_NUM_REGULATORS;
		pr_info("[DEBUG] rk3399: rk808_regulator_probe: Using RK817 regulators\n");
		break;
	case RK818_ID:
		regulators = rk818_reg;
		nregulators = RK818_NUM_REGULATORS;
		pr_info("[DEBUG] rk3399: rk808_regulator_probe: Using RK818 regulators\n");
		break;
	default:
		dev_err(&client->dev, "unsupported RK8XX ID %lu\n",
			rk808->variant);
		pr_info("[DEBUG] rk3399: rk808_regulator_probe: Unsupported variant=%lu\n", rk808->variant);
		return -EINVAL;
	}
	...
	pr_info("[DEBUG] rk3399: rk808_regulator_probe: Registering %d regulators\n", nregulators);
	...
	for (i = 0; i < nregulators; i++) {
		...
		if (IS_ERR(rk808_rdev)) {
			...
			pr_info("[DEBUG] rk3399: rk808_regulator_probe: Failed to register regulator %d, ret=%ld\n",
				i, PTR_ERR(rk808_rdev));
			return PTR_ERR(rk808_rdev);
		}
	}

	pr_info("[DEBUG] rk3399: rk808_regulator_probe: Completed successfully\n");
	return 0;
}
