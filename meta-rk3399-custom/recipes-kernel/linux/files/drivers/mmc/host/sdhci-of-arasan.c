...
static int sdhci_arasan_add_host(struct sdhci_arasan_data *sdhci_arasan)
{
	...
	pr_info("[DEBUG] mmc: sdhci_arasan_add_host: ENTRY - function called\n");
	pr_info("[DEBUG] mmc: sdhci_arasan_add_host: Starting host addition\n");

	if (!sdhci_arasan->has_cqe) {
		pr_info("[DEBUG] mmc: sdhci_arasan_add_host: No CQE, calling sdhci_add_host\n");
		ret = sdhci_add_host(host);
		pr_info("[DEBUG] mmc: sdhci_arasan_add_host: sdhci_add_host returned %d\n", ret);
		return ret;
	}
	...
	pr_info("[DEBUG] mmc: sdhci_arasan_add_host: Calling __sdhci_add_host\n");
	ret = __sdhci_add_host(host);
	if (ret) {
		pr_err("mmc: sdhci_arasan_add_host: __sdhci_add_host failed with %d\n", ret);
		goto cleanup;
	}

	pr_info("[DEBUG] mmc: sdhci_arasan_add_host: Host added successfully\n");
	return 0;
...
static int sdhci_arasan_probe(struct platform_device *pdev)
{
	...
	pr_info("[DEBUG] mmc: sdhci_arasan_probe: About to call sdhci_arasan_add_host for %s\n",
		dev_name(dev));
	ret = sdhci_arasan_add_host(sdhci_arasan);
	if (ret) {
		pr_err("mmc: sdhci_arasan_probe: sdhci_arasan_add_host failed with %d\n", ret);
		goto err_add_host;
	}

	pr_info("[DEBUG] mmc: sdhci_arasan_probe: Host added successfully, initializing wakeup\n");
	device_init_wakeup(&pdev->dev, 1);

	pr_info("[DEBUG] mmc: sdhci_arasan_probe: Probe completed successfully for %s\n",
		dev_name(dev));
	return 0;
