...
static void sdhci_set_card_detection(struct sdhci_host *host, bool enable)
{
	...
	pr_info("[DEBUG] mmc: sdhci_set_card_detection: Starting for %s (enable=%d)\n",
		mmc_hostname(host->mmc), enable);

	if ((host->quirks & SDHCI_QUIRK_BROKEN_CARD_DETECTION) ||
	    !mmc_card_is_removable(host->mmc) || mmc_can_gpio_cd(host->mmc)) {
		pr_info("mmc: sdhci_set_card_detection: Early return for %s\n",
			mmc_hostname(host->mmc));
		return;
	}

	if (enable) {
		pr_info("mmc: sdhci_set_card_detection: Reading PRESENT_STATE for %s\n",
			mmc_hostname(host->mmc));
		present = sdhci_readl(host, SDHCI_PRESENT_STATE) &
				      SDHCI_CARD_PRESENT;
		pr_info("mmc: sdhci_set_card_detection: PRESENT_STATE read, present=0x%x for %s\n",
			present, mmc_hostname(host->mmc));
	...
	pr_info("mmc: sdhci_set_card_detection: Writing INT_ENABLE for %s\n",
		mmc_hostname(host->mmc));
	sdhci_writel(host, host->ier, SDHCI_INT_ENABLE);
	pr_info("mmc: sdhci_set_card_detection: Writing SIGNAL_ENABLE for %s\n",
		mmc_hostname(host->mmc));
	sdhci_writel(host, host->ier, SDHCI_SIGNAL_ENABLE);
	pr_info("[DEBUG] mmc: sdhci_set_card_detection: Completed for %s\n",
		mmc_hostname(host->mmc));
}

static void sdhci_enable_card_detection(struct sdhci_host *host)
{
	pr_info("[DEBUG] mmc: sdhci_enable_card_detection: Starting for %s\n",
		mmc_hostname(host->mmc));
	sdhci_set_card_detection(host, true);
	pr_info("[DEBUG] mmc: sdhci_enable_card_detection: Completed for %s\n",
		mmc_hostname(host->mmc));
}
...
static void sdhci_complete_work(struct work_struct *work)
{
	...
	pr_info("[DEBUG] mmc: sdhci_complete_work: Starting for %s\n",
		mmc_hostname(host->mmc));
	while (!sdhci_request_done(host))
		;
	pr_info("[DEBUG] mmc: sdhci_complete_work: Completed for %s\n",
		mmc_hostname(host->mmc));
}
...
static irqreturn_t sdhci_irq(int irq, void *dev_id)
{
	...
	pr_info("[DEBUG] mmc: sdhci_irq: IRQ %d triggered for %s\n",
		irq, mmc_hostname(host->mmc));

	spin_lock(&host->lock);

	if (host->runtime_suspended) {
		pr_info("[DEBUG] mmc: sdhci_irq: Runtime suspended, returning for %s\n",
			mmc_hostname(host->mmc));
	...
	pr_info("[DEBUG] mmc: sdhci_irq: INT_STATUS=0x%08x for %s\n",
		intmask, mmc_hostname(host->mmc));
	if (!intmask || intmask == 0xffffffff) {
		pr_info("[DEBUG] mmc: sdhci_irq: No valid interrupt, returning for %s\n",
			mmc_hostname(host->mmc));
		result = IRQ_NONE;
		goto out;
	}
...
out:
	...
	pr_info("[DEBUG] mmc: sdhci_irq: Completed for %s, returning %d\n",
...
static irqreturn_t sdhci_thread_irq(int irq, void *dev_id)
{
	...
	pr_info("[DEBUG] mmc: sdhci_thread_irq: Starting for %s (irq=%d)\n",
	...
	isr = host->thread_isr;
	host->thread_isr = 0;
	pr_info("[DEBUG] mmc: sdhci_thread_irq: thread_isr=0x%08x for %s\n",
		isr, mmc_hostname(host->mmc));
	...
	if (isr & (SDHCI_INT_CARD_INSERT | SDHCI_INT_CARD_REMOVE)) {
		...
		pr_info("[DEBUG] mmc: sdhci_thread_irq: Card insert/remove detected for %s\n",
			mmc_hostname(mmc));
		mmc->ops->card_event(mmc);
		mmc_detect_change(mmc, msecs_to_jiffies(200));
		pr_info("[DEBUG] mmc: sdhci_thread_irq: Card event handled for %s\n",
			mmc_hostname(mmc));
	}
...
int __sdhci_add_host(struct sdhci_host *host)
{
	...
	pr_info("[DEBUG] mmc: __sdhci_add_host: Starting for %s\n",
		mmc_hostname(mmc));

	host->complete_wq = alloc_workqueue("sdhci", flags, 0);
	if (!host->complete_wq) {
		pr_err("mmc: __sdhci_add_host: Failed to alloc workqueue for %s\n",
			mmc_hostname(mmc));
		return -ENOMEM;
	}
	...
	pr_info("[DEBUG] mmc: __sdhci_add_host: Calling sdhci_init for %s\n",
		mmc_hostname(mmc));
	sdhci_init(host, 0);
	pr_info("[DEBUG] mmc: __sdhci_add_host: sdhci_init completed for %s\n",
		mmc_hostname(mmc));

	pr_info("[DEBUG] mmc: __sdhci_add_host: Requesting IRQ %d for %s\n",
		host->irq, mmc_hostname(mmc));
	...
	pr_info("[DEBUG] mmc: __sdhci_add_host: IRQ %d registered for %s\n",
		host->irq, mmc_hostname(mmc));
	...
	pr_info("[DEBUG] mmc: __sdhci_add_host: Calling mmc_add_host for %s\n",
		mmc_hostname(mmc));
	ret = mmc_add_host(mmc);
	if (ret) {
		pr_err("mmc: __sdhci_add_host: mmc_add_host failed with %d for %s\n",
			ret, mmc_hostname(mmc));
		goto unled;
	}
	pr_info("[DEBUG] mmc: __sdhci_add_host: mmc_add_host succeeded for %s\n",
		mmc_hostname(mmc));
	...
	pr_info("[DEBUG] mmc: __sdhci_add_host: DEBUG: After SDHCI controller message\n");
	pr_info("[DEBUG] mmc: __sdhci_add_host: After SDHCI controller message for %s\n",
		mmc_hostname(mmc));
	pr_info("[DEBUG] mmc: __sdhci_add_host: About to call sdhci_enable_card_detection for %s\n",
		mmc_hostname(mmc));
	pr_info("[DEBUG] mmc: __sdhci_add_host: Calling sdhci_enable_card_detection for %s\n",
		mmc_hostname(mmc));
	sdhci_enable_card_detection(host);
	pr_info("[DEBUG] mmc: __sdhci_add_host: sdhci_enable_card_detection completed for %s\n",
		mmc_hostname(mmc));
...
int sdhci_add_host(struct sdhci_host *host)
{
	...
	pr_info("[DEBUG] mmc: sdhci_add_host: Starting for %s\n",
		mmc_hostname(host->mmc));

	pr_info("[DEBUG] mmc: sdhci_add_host: Calling sdhci_setup_host\n");
	ret = sdhci_setup_host(host);
	if (ret) {
		pr_err("mmc: sdhci_add_host: sdhci_setup_host failed with %d\n", ret);
		return ret;
	}
	pr_info("[DEBUG] mmc: sdhci_add_host: sdhci_setup_host succeeded\n");

	pr_info("[DEBUG] mmc: sdhci_add_host: Calling __sdhci_add_host\n");
	ret = __sdhci_add_host(host);
	if (ret) {
		pr_err("mmc: sdhci_add_host: __sdhci_add_host failed with %d\n", ret);
		goto cleanup;
	}
	pr_info("[DEBUG] mmc: sdhci_add_host: __sdhci_add_host succeeded\n");

	pr_info("[DEBUG] mmc: sdhci_add_host: Completed successfully for %s\n",
		mmc_hostname(host->mmc));
	return 0;
