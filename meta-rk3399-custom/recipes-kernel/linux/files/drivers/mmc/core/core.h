...
{
	pr_info("[DEBUG] mmc: mmc_delay: Starting delay of %u ms\n", ms);
	if (ms <= 20) {
		pr_info("[DEBUG] mmc: mmc_delay: Calling usleep_range(%u, %u)\n",
			ms * 1000, ms * 1250);
		usleep_range(ms * 1000, ms * 1250);
		pr_info("[DEBUG] mmc: mmc_delay: usleep_range returned\n");
	} else {
		pr_info("[DEBUG] mmc: mmc_delay: Calling msleep(%u)\n", ms);
		msleep(ms);
		pr_info("[DEBUG] mmc: mmc_delay: msleep returned\n");
	}
	pr_info("[DEBUG] mmc: mmc_delay: Completed delay of %u ms\n", ms);
}
