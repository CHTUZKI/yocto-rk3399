	...
	struct mmc_card *oldcard)
{
	...
	pr_info("[DEBUG] mmc: mmc_init_card: Starting card init for %s (ocr=0x%x)\n",
	 ...
	 */
#ifndef CONFIG_ROCKCHIP_THUNDER_BOOT_MMC
	pr_info("[DEBUG] mmc: mmc_init_card: Sending GO_IDLE for %s\n",
		mmc_hostname(host));
	mmc_go_idle(host);
#endif

	/* The extra bit indicates that we support high capacity */
	pr_info("[DEBUG] mmc: mmc_init_card: Sending OP_COND with HCS bit for %s\n",
		mmc_hostname(host));
	err = mmc_send_op_cond(host, ocr | (1 << 30), &rocr);
	if (err) {
		pr_err("mmc: mmc_init_card: mmc_send_op_cond failed with %d for %s\n",
			err, mmc_hostname(host));
		goto err;
	}
	pr_info("[DEBUG] mmc: mmc_init_card: OP_COND response: 0x%x for %s\n",
		rocr, mmc_hostname(host));
	 ...
	 * Fetch CID from card.
	 */
	pr_info("[DEBUG] mmc: mmc_init_card: Fetching CID from %s\n",
		mmc_hostname(host));
	err = mmc_send_cid(host, cid);
	if (err) {
		pr_err("mmc: mmc_init_card: mmc_send_cid failed with %d for %s\n",
			err, mmc_hostname(host));
		goto err;
	}
	pr_info("[DEBUG] mmc: mmc_init_card: CID received for %s\n",
		mmc_hostname(host));
...
int mmc_attach_mmc(struct mmc_host *host)
{
	...
	pr_info("[DEBUG] mmc: mmc_attach_mmc: Starting MMC attach for %s\n",
		mmc_hostname(host));
	...
	pr_info("[DEBUG] mmc: mmc_attach_mmc: Sending OP_COND to %s\n",
		mmc_hostname(host));
	err = mmc_send_op_cond(host, 0, &ocr);
	if (err) {
		pr_err("mmc: mmc_attach_mmc: mmc_send_op_cond failed with %d for %s\n",
			err, mmc_hostname(host));
		return err;
	}
	pr_info("[DEBUG] mmc: mmc_attach_mmc: OP_COND response: 0x%x for %s\n",
		ocr, mmc_hostname(host));
	 ...
	 */
	pr_info("[DEBUG] mmc: mmc_attach_mmc: Calling mmc_init_card for %s (rocr=0x%x)\n",
		mmc_hostname(host), rocr);
	err = mmc_init_card(host, rocr, NULL);
	if (err) {
		pr_err("mmc: mmc_attach_mmc: mmc_init_card failed with %d for %s\n",
			err, mmc_hostname(host));
		goto err;
	}
	pr_info("[DEBUG] mmc: mmc_attach_mmc: mmc_init_card succeeded for %s\n",
		mmc_hostname(host));

	mmc_release_host(host);
	pr_info("[DEBUG] mmc: mmc_attach_mmc: Adding card to system for %s\n",
		mmc_hostname(host));
	err = mmc_add_card(host->card);
	if (err) {
		pr_err("mmc: mmc_attach_mmc: mmc_add_card failed with %d for %s\n",
			err, mmc_hostname(host));
		goto remove_card;
	}
	pr_info("[DEBUG] mmc: mmc_attach_mmc: Card added successfully for %s\n",
		mmc_hostname(host));
