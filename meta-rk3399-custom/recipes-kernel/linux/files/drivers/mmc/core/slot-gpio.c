...
void mmc_gpiod_request_cd_irq(struct mmc_host *host)
{
	...
	pr_info("[DEBUG] mmc: mmc_gpiod_request_cd_irq: Starting for %s\n",
		mmc_hostname(host));

	if (host->slot.cd_irq >= 0 || !ctx || !ctx->cd_gpio) {
		pr_info("[DEBUG] mmc: mmc_gpiod_request_cd_irq: Early return (cd_irq=%d, ctx=%p, cd_gpio=%p) for %s\n",
			host->slot.cd_irq, ctx, ctx ? ctx->cd_gpio : NULL, mmc_hostname(host));
		return;
	}
	...
	if (!(host->caps & MMC_CAP_NEEDS_POLL)) {
		pr_info("[DEBUG] mmc: mmc_gpiod_request_cd_irq: Getting IRQ from GPIO for %s\n",
			mmc_hostname(host));
		irq = gpiod_to_irq(ctx->cd_gpio);
		pr_info("[DEBUG] mmc: mmc_gpiod_request_cd_irq: IRQ from GPIO is %d for %s\n",
			irq, mmc_hostname(host));
	} else {
		pr_info("[DEBUG] mmc: mmc_gpiod_request_cd_irq: NEEDS_POLL set, skipping IRQ for %s\n",
			mmc_hostname(host));
	}
	...
	if (irq >= 0) {
		...
		pr_info("[DEBUG] mmc: mmc_gpiod_request_cd_irq: Requesting threaded IRQ %d for %s\n",
			irq, mmc_hostname(host));
		ret = devm_request_threaded_irq(host->parent, irq,
			...
			ctx->cd_label, host);
		if (ret < 0) {
			pr_err("[DEBUG] mmc: mmc_gpiod_request_cd_irq: Failed to request IRQ %d, error %d for %s\n",
				irq, ret, mmc_hostname(host));
			irq = ret;
		} else {
			pr_info("[DEBUG] mmc: mmc_gpiod_request_cd_irq: IRQ %d requested successfully for %s\n",
				irq, mmc_hostname(host));
		}
	}

	host->slot.cd_irq = irq;
	pr_info("[DEBUG] mmc: mmc_gpiod_request_cd_irq: cd_irq set to %d for %s\n",
		irq, mmc_hostname(host));

	if (irq < 0) {
		pr_info("[DEBUG] mmc: mmc_gpiod_request_cd_irq: Setting MMC_CAP_NEEDS_POLL for %s\n",
			mmc_hostname(host));
		host->caps |= MMC_CAP_NEEDS_POLL;
	}
	pr_info("[DEBUG] mmc: mmc_gpiod_request_cd_irq: Completed for %s\n",
		mmc_hostname(host));
}
