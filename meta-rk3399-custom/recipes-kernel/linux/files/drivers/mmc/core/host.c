...
int mmc_add_host(struct mmc_host *host)
{
	...
	pr_info("[DEBUG] mmc: mmc_add_host: Starting for %s\n",
		mmc_hostname(host));

	err = mmc_validate_host_caps(host);
	if (err) {
		pr_err("[DEBUG] mmc: mmc_add_host: mmc_validate_host_caps failed with %d for %s\n",
			err, mmc_hostname(host));
		return err;
	}
	pr_info("[DEBUG] mmc: mmc_add_host: mmc_validate_host_caps succeeded for %s\n",
		mmc_hostname(host));

	pr_info("[DEBUG] mmc: mmc_add_host: Calling device_add for %s\n",
		mmc_hostname(host));
	err = device_add(&host->class_dev);
	if (err) {
		pr_err("[DEBUG] mmc: mmc_add_host: device_add failed with %d for %s\n",
			err, mmc_hostname(host));
		return err;
	}
	pr_info("[DEBUG] mmc: mmc_add_host: device_add succeeded for %s\n",
		mmc_hostname(host));

	pr_info("[DEBUG] mmc: mmc_add_host: Registering LED trigger for %s\n",
		mmc_hostname(host));
	led_trigger_register_simple(dev_name(&host->class_dev), &host->led);
	pr_info("[DEBUG] mmc: mmc_add_host: LED trigger registered for %s\n",
		mmc_hostname(host));

#ifdef CONFIG_DEBUG_FS
	pr_info("[DEBUG] mmc: mmc_add_host: Adding debugfs for %s\n",
		mmc_hostname(host));
	mmc_add_host_debugfs(host);
	pr_info("[DEBUG] mmc: mmc_add_host: Debugfs added for %s\n",
		mmc_hostname(host));
#endif

	pr_info("[DEBUG] mmc: mmc_add_host: Calling mmc_start_host for %s\n",
		mmc_hostname(host));
	mmc_start_host(host);
	pr_info("[DEBUG] mmc: mmc_add_host: mmc_start_host completed for %s\n",
		mmc_hostname(host));

	pr_info("[DEBUG] mmc: mmc_add_host: Completed successfully for %s\n",
		mmc_hostname(host));
	return 0;
}
