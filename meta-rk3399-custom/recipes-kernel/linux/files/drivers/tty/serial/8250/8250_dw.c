...
static int dw8250_probe(struct platform_device *pdev)
{
	...
	pr_info("[DEBUG] rk3399: dw8250_probe: Starting\n");
	regs = platform_get_resource(pdev, IORESOURCE_MEM, 0);
	if (!regs) {
		pr_info("[DEBUG] rk3399: dw8250_probe: No registers defined\n");
		return dev_err_probe(dev, -EINVAL, "no registers defined\n");
	}

	pr_info("[DEBUG] rk3399: dw8250_probe: Getting IRQ\n");
	irq = platform_get_irq_optional(pdev, 0);
	/* no interrupt -> fall back to polling */
	if (irq == -ENXIO)
		irq = 0;
	if (irq < 0) {
		pr_info("[DEBUG] rk3399: dw8250_probe: Failed to get IRQ, ret=%d\n", irq);
		return irq;
	}
	pr_info("[DEBUG] rk3399: dw8250_probe: IRQ=%d, mapbase=0x%lx\n", irq, (unsigned long)regs->start);
	...
	pr_info("[DEBUG] rk3399: dw8250_probe: Mapping registers\n");
	p->membase = devm_ioremap(dev, regs->start, resource_size(regs));
	if (!p->membase) {
		pr_info("[DEBUG] rk3399: dw8250_probe: Failed to map registers\n");
		return -ENOMEM;
	}

	data = devm_kzalloc(dev, sizeof(*data), GFP_KERNEL);
	if (!data) {
		pr_info("[DEBUG] rk3399: dw8250_probe: Failed to allocate data\n");
		return -ENOMEM;
	}
	pr_info("[DEBUG] rk3399: dw8250_probe: Data allocated, initializing\n");
	...
	pr_info("[DEBUG] rk3399: dw8250_probe: Registering 8250 port\n");
	data->data.line = serial8250_register_8250_port(up);
	if (data->data.line < 0) {
		pr_info("[DEBUG] rk3399: dw8250_probe: Failed to register 8250 port, ret=%d\n", data->data.line);
		return data->data.line;
	}
	pr_info("[DEBUG] rk3399: dw8250_probe: Registered 8250 port, line=%d\n", data->data.line);
	...
	pr_info("[DEBUG] rk3399: dw8250_probe: Completed successfully\n");
	return 0;
}
