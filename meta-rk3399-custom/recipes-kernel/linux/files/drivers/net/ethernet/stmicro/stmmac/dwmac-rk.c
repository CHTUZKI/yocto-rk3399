...
static int rk_gmac_probe(struct platform_device *pdev)
{
	...
	pr_info("[DEBUG] rk3399: rk_gmac_probe: Starting\n");
	data = of_device_get_match_data(&pdev->dev);
	if (!data) {
		dev_err(&pdev->dev, "no of match data provided\n");
		pr_info("[DEBUG] rk3399: rk_gmac_probe: No match data\n");
		return -EINVAL;
	}

	pr_info("[DEBUG] rk3399: rk_gmac_probe: Getting platform resources\n");
	ret = stmmac_get_platform_resources(pdev, &stmmac_res);
	if (ret) {
		pr_info("[DEBUG] rk3399: rk_gmac_probe: Failed to get platform resources, ret=%d\n", ret);
		return ret;
	}

	pr_info("[DEBUG] rk3399: rk_gmac_probe: Probing config from DT\n");
	plat_dat = stmmac_probe_config_dt(pdev, stmmac_res.mac);
	if (IS_ERR(plat_dat)) {
		pr_info("[DEBUG] rk3399: rk_gmac_probe: Failed to probe config, ret=%ld\n", PTR_ERR(plat_dat));
	...
	pr_info("[DEBUG] rk3399: rk_gmac_probe: Setting up GMAC\n");
	plat_dat->bsp_priv = rk_gmac_setup(pdev, plat_dat, data);
	if (IS_ERR(plat_dat->bsp_priv)) {
		ret = PTR_ERR(plat_dat->bsp_priv);
		pr_info("[DEBUG] rk3399: rk_gmac_probe: Failed to setup GMAC, ret=%d\n", ret);
		goto err_remove_config_dt;
	}

	pr_info("[DEBUG] rk3399: rk_gmac_probe: Initializing CSU\n");
	rk_gmac_csu_init(plat_dat);

	pr_info("[DEBUG] rk3399: rk_gmac_probe: Initializing clocks\n");
	ret = rk_gmac_clk_init(plat_dat);
	if (ret) {
		pr_info("[DEBUG] rk3399: rk_gmac_probe: Failed to init clocks, ret=%d\n", ret);
		goto err_remove_config_dt;
	}

	pr_info("[DEBUG] rk3399: rk_gmac_probe: Powering up GMAC\n");
	ret = rk_gmac_powerup(plat_dat->bsp_priv);
	if (ret) {
		pr_info("[DEBUG] rk3399: rk_gmac_probe: Failed to powerup, ret=%d\n", ret);
		goto err_remove_config_dt;
	}

	pr_info("[DEBUG] rk3399: rk_gmac_probe: Probing STMMAC driver\n");
	ret = stmmac_dvr_probe(&pdev->dev, plat_dat, &stmmac_res);
	if (ret) {
		pr_info("[DEBUG] rk3399: rk_gmac_probe: Failed to probe STMMAC, ret=%d\n", ret);
		goto err_gmac_powerdown;
	}

	pr_info("[DEBUG] rk3399: rk_gmac_probe: Creating loopback sysfs\n");
	ret = dwmac_rk_create_loopback_sysfs(&pdev->dev);
	if (ret) {
		pr_info("[DEBUG] rk3399: rk_gmac_probe: Failed to create sysfs, ret=%d\n", ret);
		goto err_gmac_powerdown;
	}

	pr_info("[DEBUG] rk3399: rk_gmac_probe: Completed successfully\n");
	return 0;
