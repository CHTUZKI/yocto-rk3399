...
static void __init init_port(struct earlycon_device *device)
{
	...
	if (port->uartclk) {
		...
		/* DEBUG: For RK3399 UART2, verify divisor calculation */
		if (port->mapbase == 0xff1a0000UL) {
			/* Expected: uartclk=24000000, baud=1500000, divisor=1 */
			/* divisor = 24000000 / (16 * 1500000) = 1 */
		}
					 ...
					 const char *options)
{
	...
	if (device->port.mapbase == 0xff1a0000UL) {
		/* DEBUG: Always force uartclk to 24MHz for RK3399 UART2 */
		device->port.uartclk = 24000000; /* 24MHz for RK3399 UART2 */
		/* Force baud rate to 1500000 if not set */
		if (!device->baud) {
			device->baud = 1500000;
		}
	}
